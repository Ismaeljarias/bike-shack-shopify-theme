  ...
  loadContent(focus = true) {
    ...
    if (!this.getAttribute("loaded")) {
      ...
      if (
        ...
        // deferredElement.play(); original code
        deferredElement
          .play()
          .then(() => {
            this.dispatchEvent(
              new CustomEvent("videoLoaded", {
                detail: { videoElement: deferredElement },
                bubbles: true, // This makes the event bubble up through the DOM
                cancelable: true, // This allows the event to be cancellable
                composed: false, // Set to true if you want the event to cross the shadow DOM boundary
              })
            );
          })
